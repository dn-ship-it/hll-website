// ripple-shaders.ts — GLSL for the "HLL Ripples" p5.js sketch, copied
// verbatim from HLL-UI-Demo. Its ripple and corner-ripple folders ship
// byte-identical shader.vert/shader.frag pairs, so both read this one module.
//
...
export const RIPPLE_VERT_SRC = `precision highp float;
...
export const RIPPLE_FRAG_SRC = `precision highp float;
...
if (!uTapRippleMode) {
    ...
    float angularWarp =
        sin(rippleP.x * 2.7 + rippleP.y * 1.3) *
        cos(rippleP.y * 2.1 - rippleP.x * 1.7);

    r += angularWarp * 0.006;

    float circularNoise =
        sin(r * 12.0 - t * 2.0) * 0.04;

    float ovalNoise =
        sin(rippleP.x * 5.0 + t) * 0.085 +
        cos(rippleP.y * 5.0 - t) * 0.015;
    ...
    float rippleNoise =
        sin(rippleP.x * 18.0 + t * 1.2) *
        cos(rippleP.y * 18.0 - t * 0.9);

    rippleNoise +=
        sin((rippleP.x + rippleP.y) * 25.0 - t);
    ...
    vec2 rawFlow = vec2(flow, flow2);

    float flowLen =
        max(length(rawFlow), 0.25);

    vec2 flowDir =
    rawFlow /
    max(length(rawFlow), 0.5);
